...
export default function DefaultMarketplaceDetailsPage({ address, selectedChainId, account }: DefaultMarketplaceDetailsPageProps) {
  ...
  useEffect(() => {
    const getNft = async () => {
      if (address && selectedChainId && !updateListingModal) {
        setIsLoading(true)
        const nft =
          (await marketplaceService(selectedChainId, 2).getMarketplaceItemByAddress(address)) ||
          (await marketplaceService(selectedChainId, 1).getMarketplaceItemByAddress(address))
        if (!nft) {
          setErc20(null)
          setIsLoading(false)
          return
        ...
        if (String(nftWithLiquidity?.target.collection.id) === String(boxAddress.toLowerCase())) {
          setIsBox(true)
          const nftsInsideBox = await getBoxMetadata(Number(nftWithLiquidity?.target.tokenId), selectedChainId)
  ...
  useEffect(() => {
    const getShareBalance = async () => {
      if (account && erc20?.id) {
        const balance = await getErc20Balance(account, erc20?.id, erc20?.decimals, selectedChainId)
  ...
  useEffect(() => {
    const getPaymentTokenBalance = async () => {
      if (account && erc20?.paymentToken?.id && erc20?.paymentToken?.decimals) {
        const balance = await getErc20Balance(account, erc20?.paymentToken.id, erc20?.paymentToken.decimals, selectedChainId)
  ...
  return (
    <DefaultPageTemplate>
      <div id='details'>
        ...
        {isBox && (
          <>
            <S.TitleBoxContent>NFTs in this Box</S.TitleBoxContent>
            <S.Slider {...settingsSlider}>
              {nftsIntroBox?.nfts.map(nftItem => {
                return (
                  <S.CardBox key={nftItem.tokenId}>
                    <Image src={nftItem.image_url} alt={nftItem.name} fallback={notFount} />
                  </S.CardBox>
                )
              })}
            </S.Slider>
