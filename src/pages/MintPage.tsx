...
export default function MintPage() {
  ...
  const routeNftType = history.location.pathname.split('/')[2] as 'image' | 'video' | 'audio'
  ...
  useEffect(() => {
    if (!transactionLoading && transaction && transaction.type === 'mint' && transaction.confirmed) {
  ...
  const mintNft = async () => {
    ...
    if (nftType === 'video' || nftType === 'audio') {
      properties['preview_media_file_type2'] = {
        type: 'string',
    ...
    nftImage && !nftVideo && (nftMetadataJson.image = '$media-uri')
    nftImage && (nftVideo || nftAudio) && (nftMetadataJson.image = '$preview-uri')
    nftImage && nftVideo && (nftMetadataJson.animation_url = '$video-uri')
    nftImage && nftAudio && (nftMetadataJson.animation_url = '$media-uri')

    const nftMetadataFile: Blob = new Blob([JSON.stringify(nftMetadataJson)], { type: 'application/json' })

    const nftMetadata = nftImage && (await PinataIpfsService(chainId).uploadToIpfs(nftMetadataFile, nftImage, nftVideo, nftAudio))
    ...
    account && nftMetadata.cid && mintErc721(mint.defaultCollectionAddress, nftMetadata.cid, account, chainId)
    setIsLoading(false)
  }
  ...
  const handleSetNftType = (selectedNftType: 'video' | 'image' | 'audio') => {
    history.push(`/create/${selectedNftType}`)
    setNftVideo(undefined)
    setNftAudio(undefined)
...
const S = {
  ...
  NftTypeWrapper: styled.div`
    ...
    gap: 24px;
    @media (min-height: ${viewportV2.mobile}) and (max-width: ${viewportV2.tablet}) {
      gap: 10px;
    }
    > button {
      border: none;
