...
import { getBoxItemById } from './BoxService'
...
export const collectiveBuyService = (): CollectiveBuyService => {
  const setItemMetadata = async (chainId: number, item: CollectiveBuyMarketplaceItem): Promise<CollectiveBuyMarketplaceItem> => {
    ...
    if (erc721?.address.toLowerCase() === boxAddress.toLowerCase()) {
      const { box } = await getBoxItemById(String(erc721.tokenId), chainId)
      return {
        ...item,
        boxCount: box.items.length,
        erc721
      }
    }
