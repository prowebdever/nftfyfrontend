...
export const getErc721Metadata = async (address: string, tokenId: string, web3: Web3) => {
  ...
  try {
    ...
    const { name, description, image, imageUrl, previewUrl } = metadata.data
    return {
      description,
      image_url: safeIpfsUrl(image || imageUrl || previewUrl || ''),
      ...
      animationType:
        metadata.data?.properties?.preview_media_file_type2?.description || metadata.data?.properties?.preview_media_file2_type?.description
    }
  } catch (error) {
