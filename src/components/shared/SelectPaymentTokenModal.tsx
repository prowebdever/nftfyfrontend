...
export const SelectPaymentTokenModal = ({ visible, onCancel, onSelect, location }: AssetsTokenModalProps) => {
  ...
  const getAssetsTokens = useCallback(async () => {
    if (account) {
      const erc20 =
        location === 'marketplace' ? await getMarketplaceAssetsList(account, chainId) : await getFractionalizeAssetsList(account, chainId)
      setAssetsFilter(erc20)
    }
  }, [account, chainId, location])
      ...
      if (assetsFilter && e.target.value) {
        ...
        if (!tokens.length) {
          ...
          if (token) {
            ...
            if (account) {
              const balance = await getErc20Balance(account, token.address, token.decimals, chainId)
              setAssetsFilter([{ ...token, balance: balance.toString() }])
            } else {
              setAssetsFilter([{ ...token, balance: '0' }])
            }
          } else {
            setAssetsFilter([])
          }
          ...
          assetsFilter.map(asset => (
            <li key={asset.id}>
              <S.ButtonToken onClick={() => handleSelect(asset)}>
                ...
                {account && <span>{asset.balance && Number(asset.balance).toLocaleString('en-us', { maximumFractionDigits: 6 })}</span>}
              </S.ButtonToken>
            </li>
          ))}
      </S.ListAssets>
