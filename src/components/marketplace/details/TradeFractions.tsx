...
export function TradeFractions({ erc20 }: TradeFractionsProps) {
  ...
  useEffect(() => {
    const getShareBalance = async () => {
      if (account && erc20?.id) {
        const balance = await getErc20Balance(account, erc20?.id, erc20?.decimals, chainId)
  ...
  useEffect(() => {
    const getPaymentTokenBalance = async () => {
      if (account && erc20?.paymentToken?.id && erc20?.paymentToken?.decimals) {
        const balance = await getErc20Balance(account, erc20?.paymentToken.id, erc20?.paymentToken.decimals, chainId)
      ...
      <article>
        <S.Info>
          ...
          <div>
            ...
            <span>
              <span>Holders</span>
              <a rel='noreferrer' target='_blank' href={`${etherscanAddress}/address/${erc20.id}`}>
