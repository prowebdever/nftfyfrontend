...
export function BuyModalNft({ erc20, account }: BuyModalNftProps) {
  ...
  useEffect(() => {
    const getShareBalance = async () => {
      if (account) {
        const balance = await getErc20Balance(account, id, decimals, chainId)
  ...
  useEffect(() => {
    const getPaymentTokenBalance = async () => {
      if (account) {
        const balance = await getErc20Balance(account, paymentToken.id, paymentToken.decimals, chainId)
