...
export const ClaimModal: React.FC<ClaimModalProps> = ({ className }: ClaimModalProps) => {
  ...
  useEffect(() => {
    const getShareBalance = async () => {
      if (account && erc20Share) {
        const balance = await getErc20Balance(account, erc20Share.id, erc20Share.decimals, chainId)
        setErc20Balance(balance.toString())
      }
    }
    getShareBalance()
  }, [account, chainId, erc20Share])

  useEffect(() => {
    const getVaultBalance = async () => {
      if (account && erc20Share) {
        const balance = await getAccountClaimAmount(erc20Share.id, account, chainId)
  ...
  const claim = async () => {
    setLoading(true)
    if (erc20Share && account) {
      claimErc20(erc20Share.id, account, chainId)
    }
    setLoading(false)
  }
